...
if [ -n "$CURRENT_VERSION" ] && [ "$CURRENT_VERSION" = "$LATEST_VERSION" ] && [ -x "$EXECUTABLE" ]; then
    print_success "MongoDB Compass v$LATEST_VERSION is up to date"
    exit 0
fi

if [ -n "$CURRENT_VERSION" ] && [ "$CURRENT_VERSION" = "$LATEST_VERSION" ] && [ ! -x "$EXECUTABLE" ]; then
    print_info "La versión v$LATEST_VERSION figura instalada, pero falta el ejecutable. Reinstalando..."
fi
